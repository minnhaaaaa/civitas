...
class DemoExecutionLedger:
    ...
    async def execute(
        ...
        plan_hash = selected_plan_hash(plan)
...
def _align_request_to_goal(
    ...
    raw_lots = constraints.get("inventory_lots", [])
    if not isinstance(raw_lots, list):
        raise _product_error(ProductErrorCode.CONFLICT, "demo inventory lots are malformed")
    lots = [dict(item) for item in raw_lots if isinstance(item, dict)]
    for lot in lots:
        # The fixture represents fresh on-hand stock. Keep that invariant relative
        # to the requested horizon instead of relying on a calendar date that ages.
        lot["expires_at"] = (goal.horizon_ends_at + duration).isoformat()
    constraints["inventory_lots"] = cast(JsonValue, lots)
